...
startTime<-Sys.time()
rfModelFit<-train(model.training$classe ~ ., data=model.train.PC, method="rf", prox=TRUE, trControl=trainControl(method="cv",allowParallel=TRUE,),tuneGrid=expand.grid(mtry = 10),number=3,ntree=500,do.trace=TRUE)
endTime<-Sys.time()
