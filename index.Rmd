...
In This section we will try to get an Insight into data in order to reduce the number of features. To do so we will use pca to extract the most important features which have the most infomration. This will be very usefull when using algorithms such as random forest. Since High number of features causes very tall Tree hence very long execution time. 
